    ...
    <div>
        <header>
            ...
            <div class="time" v-show="haveclock">
                <div >倒计时</div> 
                <van-count-down ref="clock" :time="time" format="mm:ss" @finish="finishtime"/>
            </div>
        </header>
        <van-popup v-model="showList"  position="bottom" :style="{ width: '100%', 'max-height':'71%' }" duration='.5'>
            <div class="upfoot">
                    ...
                    <div class="fright">
                        <span>
                            <img src="http://j2.58cdn.com.cn/jxedt/m/kaoshi/images/lx_item_state_right.png" class="small_img">
                            {{0}}
                        </span>
                        <span>
                            <img src="http://j2.58cdn.com.cn/jxedt/m/kaoshi/images/lx_item_state_wrong.png" class="small_img">
                            {{0}}
                        </span>
                        <span>
                            <svg class="icon" aria-hidden="true" >
            ...
            <ul class="upitem">
                <li v-for="(item,i) in rdques" :key="i" class="item" :class="[{thisitem: index === i}, {selectovertrue: selectres(i) === 2}, {selectoverfalse: selectres(i) === 1 }]" @click="clickupitem(i)">{{i+1}}</li>
            </ul>
        </van-popup>
        <van-popup v-model="submitshow" round :style="{width:'300px'}">
            ...
            <div class="begin_buttom">
                ...
                <div class="certain" @click="certainsub" :class="{onlycertain:  time == 0}">
                    {{certain}}
                </div>
            </div>
        </van-popup>
        ...
        <div v-if="!submittest">
            <div class="test" v-if="show">
                <div class="test-title">
                    ...
                    <div v-if="data.imageurl != ''" class="picture"><img :src="data.imageurl"></div>
                    <selectOption v-for="(el,i) in option" :key= i :item="i" :option="el" :selectOption="selectOption" :select="select" :rightOption="data.answertrue" @click="clickOption(i+1)"></selectOption>
                    <div style="padding: 20px 10px" v-if="data.answertrue.length > 1 && select == false" @click="refer">
            ...
            <div class="handelBtns">
                <div class="pre" :class="{cantBtn: index <= 0}" @click="pre">上一题</div>
                <div class="next" :class="{cantBtn: index >= rdques.length - 1}" @click="next">下一题</div>
            </div>
            <footer>
        ...
        <div class="mnks-result" v-else>
            <img src="http://img.58cdn.com.cn/dist/jxedt/h5/m_new/img/mnks-result.png">
            <div class="score">
    ...
    methods: {
        ...
        refer() {
            if(this.selectOption.length > 1){
                ...
                this.datas.push({id: this.data.id, select: this.selectOption, index: this.index, res: result})
                // 回答完后直接跳转
                this.trueres ++
                let that = this.next
                setTimeout(function(){
...
<style lang="scss" scoped>
    ...
    .upitem {
        ...
        .selectovertrue {
            background: rgba(0,187,126,.2);
            border: 1px solid #00bb7e;
            color: #00bb7e;
        }

        .selectoverfalse {
            background: rgba(251,110,82,.2);
            border: 0;
            color: #fb6e52;
        }
    }
