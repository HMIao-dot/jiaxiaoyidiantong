<template>
    <div>
        ...
        <van-popup v-model="showList"  position="bottom" :style="{ width: '100%', 'max-height':'71%' }" duration='.5'>
            ...
            <ul class="upitem">
                <li v-for="(item,i) in wrongquestions" :key="i" class="item" :class="[{thisitem: index === i}, {selectovertrue: selectres(i) === 2}, {selectoverfalse: selectres(i) === 1 }]" @click="clickupitem(i)">{{i+1}}</li>
            </ul>
        </van-popup>
        ...
        <div class="handelBtns">
            ...
            <div class="next" :class="{cantBtn: index >= wrongquestions.length - 1}" @click="next">下一题</div>
        </div>
        <footer>
...
export default {    
    ...
    methods: {
        ...
        fetchdata() {
            this.show = false;
            let el = this.datas.find((item) => item.id === this.wrongquestions[this.index])
            if(el) {
                // data: Object,
                fetch(`/g.asp?id=${this.wrongquestions[this.index]}`)
                // fetch(`/g.asp?id=1550`)
