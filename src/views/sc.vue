<template>
    <div>
        ...
        <van-popup v-model="showList"  position="bottom" :style="{ width: '100%', 'max-height':'71%' }" duration='.5'>
            ...
            <ul class="upitem">
                <li v-for="(item,i) in collects" :key="i" class="item" :class="[{thisitem: index === i}, {selectovertrue: selectres(i) === 2}, {selectoverfalse: selectres(i) === 1 }]" @click="clickupitem(i)">{{i+1}}</li>
            </ul>
        </van-popup>
        ...
        <div class="handelBtns">
            ...
            <div class="next" :class="{cantBtn: index >= collects.length - 1}" @click="next">下一题</div>
        </div>
        <footer>
            <div class="fleft" @click='collectSelect'>
                <svg class="icon icon-big" aria-hidden="true" v-if="collect">
